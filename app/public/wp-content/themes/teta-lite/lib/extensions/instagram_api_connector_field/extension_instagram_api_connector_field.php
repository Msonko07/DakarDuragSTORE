...
// Don't duplicate me!
if ( ! class_exists( 'ReduxFramework_Extension_instagram_api_connector_field' ) ) {
	/**
	 * Main ReduxFramework custom_field extension class
	 *
	 * @since       3.1.6
	 */
	class ReduxFramework_Extension_instagram_api_connector_field extends ReduxFramework {

		// Set the version number of your extension here
		public static $version = '1.0.0';
		// Set the name of your extension here
		public $ext_name = 'instagram_api_connector';
		...
		public function __construct( $parent ) {
			...
			$this->field_name  = 'instagram_api_connector';
		...
		private function is_minimum_version() {
			...
			if ( $this->min_redux_version != '' ) {
				if ( version_compare( $redux_ver, $this->min_redux_version ) < 0 ) {
					$msg = '<strong>' . esc_html__( 'The', 'teta-lite' ) . ' ' . $this->ext_name . ' ' . esc_html__( 'extension requires', 'teta-lite' ) . ' Redux Framework ' . esc_html__( 'version', 'teta-lite' ) . ' ' . $this->min_redux_version . ' ' . esc_html__( 'or higher.', 'teta-lite' ) . '</strong>&nbsp;&nbsp;' . esc_html__( 'You are currently running', 'teta-lite' ) . ' Redux Framework ' . esc_html__( 'version', 'teta-lite' ) . ' ' . $redux_ver . '.<br/><br/>' . esc_html__( 'This field will not render in your option panel, and featuress of this extension will not be available until the latest version of', 'teta-lite' ) . ' Redux Framework ' . esc_html__( 'has been installed.', 'teta-lite' );
					...
					if ( method_exists( 'Redux_Admin_Notices', 'set_notice' ) ) {
						...
						echo '<p>';
						echo wp_kses( $msg, kite_allowed_html() );
						echo '</p>';
						echo '</div>';
					}
					return false;
